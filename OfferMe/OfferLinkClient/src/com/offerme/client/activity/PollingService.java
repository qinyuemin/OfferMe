...
public class PollingService extends Service {
	...
	private boolean hasAliasID = false;
	...
	private void initPersonalInfo() {
		...
		personInfo.setFirstFeedBack(true);
	}
	...
	private void initPushAgent() {
		...
		mPushAgent.disable();
		String value = personInfo.getValue(LocalPersonInfo.ACCEPT_MESSAGE);
	...
	private void doVibre() {
		Vibrator vibrate = (Vibrator) this
	...
	private boolean acceptMessage(Integer userID) {
		String value = personInfo.getValue(LocalPersonInfo.ACCEPT_MESSAGE);
		if (userID == 0 && value != null && !Boolean.parseBoolean(value)) {
			return false;
		}
		return true;
	}
	...
	private class PollingThread extends Thread {
		...
		private void newSystemMessageSwitch() {
			try {
				if (mPushAgent.isRegistered() && !hasAliasID) {
					String userid = personInfo.getValue(LocalPersonInfo.USERID);
					hasAliasID = true;
					mPushAgent.addAlias(userid, "OFFERLINK_USERID");
				}
			} catch (Exception e) {
				// //e.printStackTrace();
			}
